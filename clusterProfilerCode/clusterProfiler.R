...
## BRCA dataset
#load data
model.results<- read_csv("../geneListFilter/filtered/filtered_BRCA_DEseq_list.csv")
#Define significant gene
signif<-model.results
signif.entrez<-unique(signif$SYMBOL)

#Analyse using custom_BP
custom_BP <-read.gmt("../customGMT/go-bp_2021_gene_set.gmt")
...
custom_MF <-read.gmt("../customGMT/go-mf_2021_gene_set.gmt")
...
## BLCA dataset
#load data
model.results<- read_csv("../geneListFilter/filtered/filtered_BLCA_DEseq_list.csv")
#Define significant gene
